...
function fetchActivityTypes(): Promise<ActivityType[]> {
...
export default function ActivityTypes() {
  ...
  const create = useMutation({
    mutationFn: (name: string) =>
      apiFetch('/activity-types', { method: 'POST', body: JSON.stringify({ name }) }),
    onSuccess: () => {
  ...
  const update = useMutation({
    mutationFn: ({ id, name }: { id: number; name: string }) =>
      apiFetch(`/activity-types/${id}`, { method: 'PUT', body: JSON.stringify({ name }) }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['activity-types'] })
      setEditingId(null)
    },
  ...
  if (isLoading) return <div className="p-6 text-text-secondary">Loading activity types...</div>

  const items = types ?? []

  return (
    <div className="p-6 max-w-2xl">
      <h2 className="text-xl font-bold text-text-primary mb-4">Activity Types</h2>
      ...
      <div className="bg-bg-card border border-border rounded-lg overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-bg-secondary">
              ...
              <th className="text-right px-3 py-2 text-xs text-text-secondary font-medium">Activities</th>
          ...
          <tbody>
            {items.map(t => (
              <tr key={t.id} className="border-b border-border last:border-0 hover:bg-bg-hover">
                ...
                <td className="px-3 py-2 text-right text-text-secondary">{t.activity_count}</td>
        ...
        {items.length === 0 && (
          <div className="text-center text-text-secondary py-8">No activity types defined</div>
        )}
      </div>
